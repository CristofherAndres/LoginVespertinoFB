...
import com.example.loginvespertinofb.databinding.ActivityRegistroBinding
...
class RegistroActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Reconocer el boton para registrar un usuario
        binding.btnRegistrar.setOnClickListener {

            //Obtener el correo, password y la confirmacion de la password
            val email:String = binding.etCorreo.text.toString()
            val password:String = binding.etPassword.text.toString()
            val conPassword:String = binding.etPassword2.text.toString()

            //Informar de errores si los campos son vacios
            if(email.isEmpty()) {
